document.addEventListener("DOMContentLoaded", async () => {
  ...
  try {
    const response = await fetch('../components/popular-reviews/data.json')
    ...
    const articles = data.map((item, index) => {
      ...
      const mediaYear = document.createElement('h6')
      mediaYear.classList.add('main-info__media-year')
      mediaYear.textContent = item.year
      mediaDetails.appendChild(mediaYear)
